...
import { TExternalStatus, TOverviewView, TTask, TTaskFolder, TTaskV4, TUserInfo } from '../../utils/types'
...
export const useCreateTask = () => {
    ...
    return useGTMutation((data: TCreateTaskData) => createTask(data), {
        tag: 'tasks',
        invalidateTagsOnSettled: ['folders', 'tasks', 'overview'],
        onMutate: async (data: TCreateTaskData) => {
            ...
            if (views) {
                const updatedViews = produce(views, (draft) => {
                    const folder = draft.find((view) => view.task_section_id === data.id_folder)
                    if (!folder) return
                    folder.view_item_ids = [data.optimisticId, ...folder.view_item_ids]
                })
        ...
        onSuccess: async (response: TCreateTaskResponse, createData: TCreateTaskData) => {
            ...
            if (views) {
                const updatedViews = produce(views, (draft) => {
                    const view = draft.find((view) => view.task_section_id === createData.id_folder)
                    if (!view) return
                    const taskIdIndex = view.view_item_ids.indexOf(createData.optimisticId)
                    if (taskIdIndex === -1) return
                    view.view_item_ids[taskIdIndex] = response.task_id
                })
                queryClient.setQueryData('overview', updatedViews)
            }
...
export const useMarkTaskDoneOrDeleted = () => {
    ...
    return useGTMutation((data: TMarkTaskDoneOrDeletedData) => markTaskDoneOrDeleted(data), {
        tag: 'tasks',
        invalidateTagsOnSettled: ['tasks', 'folders', 'overview'],
...
export const useReorderTask = () => {
    ...
    return useGTMutation((data: TReorderTaskData) => reorderTask(data), {
        ...
        onMutate: async (data: TReorderTaskData) => {
            ...
            if (data.dropSectionId === data.dragSectionId) return
