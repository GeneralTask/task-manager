...
export default function TaskSectionHeader(props: Props): JSX.Element {
    ...
    const [{ isOver }, drop] = useDrop(() => ({
        ...
        drop: ({ id, indicesRef }: { id: string; indicesRef: RefObject<Indices> }) => {
            ...
            const updatedTaskSections = sectionDropReorder(
                taskSectionsRef.current,
                props.task_section_index,
                indicesRef.current
            )
            ...
            const patchBody = JSON.stringify({
                ...
                id_ordering: updatedTaskSections[props.task_section_index].tasks[0].id_ordering,
            })
    ...
    return (
        <TaskSectionHeaderContainer>
            ...
            <TimeAnnotationRight>{props.isToday && <RefreshButton />}</TimeAnnotationRight>
        </TaskSectionHeaderContainer>
    )
}
