...
const TimeEstimateAction = ({
    sourceName,
    taskId,
    timeAllocated,
    isSelected,
}: TimeEstimateActionProps): JSX.Element => {
    ...
    const timeEstimateView =
        timeAllocated >= DEFAULT_ALLOCATION || timeAllocated === 0 ? (
    ...
    return (
        <>
            ...
            <Tooltip text={'Time Estimate'}>
                <Action onClick={onClick}>{timeEstimateView}</Action>
            </Tooltip>
...
const DueDateAction = ({ taskId, dueDate, isSelected }: DueDateActionProps): JSX.Element => {
    ...
    const simpleDueDate = new Date(new Date(dueDate).valueOf() + 86400000).toLocaleDateString('default', {
        day: 'numeric',
        month: 'short',
    })
...
const HeaderActions = (props: HeaderActionsProps) => {
    ...
    if (props.isOver || props.isSelected) {
        actions = [
            <TimeEstimateAction
                sourceName="General Task"
                taskId={props.task.id}
                timeAllocated={props.task.time_allocated}
