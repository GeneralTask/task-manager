...
import { ButtonIcon, ButtonRight, HeaderLeft, HeaderRight, TaskHeaderContainer, Title } from '../task/TaskHeader-style'
import { CONVERT_ICON, MESSAGES_MODIFY_URL } from '../../constants'
            ...
            <HeaderRight>
                ...
                }
                {
                    <ButtonRight onClick={(e) => {
                        e.stopPropagation()
                        convertToTask(props.message.id, fetchMessages)
                    }}>
                        <ButtonIcon src={CONVERT_ICON} alt='Convert to Task' />
                    </ButtonRight>
                }
...
const convertToTask = async (id: string, fetchMessages: () => void) => {
    try {
        const response = await makeAuthorizedRequest({
            url: MESSAGES_MODIFY_URL + id + '/',
            method: 'PATCH',
