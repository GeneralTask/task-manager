...
func TestLinkAsanaCallback(t *testing.T) {
	...
	t.Run("UnsuccessfulResponse", func(t *testing.T) {
		server := getTokenServerForAsana(t, http.StatusUnauthorized)
		api := GetAPI()
	...
	t.Run("Success", func(t *testing.T) {
		server := getTokenServerForAsana(t, http.StatusOK)
		api := GetAPI()
		(api.ExternalConfig.Asana.(*external.OauthConfig)).Config.Endpoint.TokenURL = server.URL
...
func getTokenServerForAsana(t *testing.T, statusCode int) *httptest.Server {
