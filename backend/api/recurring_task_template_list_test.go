...
func TestRecurringTaskTemplateList(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		...
		assert.Equal(t,
			fmt.Sprintf(`[{"ID":"%s","UserID":"%s","Title":"recurring task!","Body":null,"IDTaskSection":"000000000000000000000000","PriorityNormalized":3,"IsEnabled":true,"IsDeleted":false,"RecurrenceRate":0,"TimeOfDaySecondsToCreateTask":null,"DayToCreateTask":null,"MonthToCreateTask":null,"LastBackfillDatetime":"%s"},{"ID":"%s","UserID":"%s","Title":"recurring task!","Body":null,"IDTaskSection":"000000000000000000000000","PriorityNormalized":3,"IsEnabled":false,"IsDeleted":false,"RecurrenceRate":0,"TimeOfDaySecondsToCreateTask":null,"DayToCreateTask":null,"MonthToCreateTask":null,"LastBackfillDatetime":"%s"}]`, template1Result.InsertedID.(primitive.ObjectID).Hex(), userID.Hex(), triggerTime.Format("2006-01-02T15:04:05.999Z07:00"), template2Result.InsertedID.(primitive.ObjectID).Hex(), userID.Hex(), triggerTime.Format("2006-01-02T15:04:05.999Z07:00")),
			string(body))
	})
}
