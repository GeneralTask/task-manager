...
func TestTaskDetail(t *testing.T) {
	...
	t.Run("SuccessTaskFromEmail", func(t *testing.T) {
		...
		body := ServeRequest(t, authToken, "GET", fmt.Sprintf("/tasks/detail/%s/", task.ID.Hex()),
			nil, http.StatusOK)
		assert.Equal(t,
			fmt.Sprintf(`{"id":"%s","id_ordering":0,"source":{"name":"General Task","logo":"/images/generaltask.svg","logo_v2":"generaltask","is_completable":true,"is_replyable":false},"deeplink":"","title":"sample title","body":"sample body","sender":"","due_date":"","time_allocated":0,"sent_at":"1970-01-01T00:00:00Z","is_done":false,"linked_email_thread":{"linked_thread_id":"%s","linked_email_id":"%s","emails":[{"message_id":"%s","subject":"test subject 1","body":"test body 1","sent_at":"2019-04-20T00:00:00Z","is_unread":true,"sender":{"name":"test","email":"test@generaltask.com","reply_to":"test-reply@generaltask.com"},"recipients":{"to":[{"name":"p1","email":"p1@gmail.com"}],"cc":[{"name":"p2","email":"p2@gmail.com"}],"bcc":[{"name":"p3","email":"p3@gmail.com"}]}},{"message_id":"000000000000000000000000","subject":"test subject 2","body":"test body 2","sent_at":"2018-04-20T00:00:00Z","is_unread":true,"sender":{"name":"test","email":"test@generaltask.com","reply_to":""},"recipients":{"to":[],"cc":[],"bcc":[]}}]}}`,
				task.ID.Hex(), threadID.Hex(), firstEmailID.Hex(), firstEmailID.Hex()),
			string(body))
	})
}
