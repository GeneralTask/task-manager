...
func (api *API) TaskModify(c *gin.Context) {
	...
	// handle reorder task
	if modifyParams.IDOrdering != nil || (modifyParams.IDTaskSection != nil || (task.ParentTaskIDHex != nil && *task.ParentTaskIDHex != "")) {
...
func (api *API) ReOrderTask(c *gin.Context, taskID primitive.ObjectID, userID primitive.ObjectID, IDOrdering *int, IDTaskSectionHex *string, task *database.Task) error {
	...
	if task.ParentTaskIDHex != nil && *task.ParentTaskIDHex != "" {
		dbQuery = append(dbQuery, bson.M{"parent_task_id": task.ParentTaskIDHex})
	} else {
